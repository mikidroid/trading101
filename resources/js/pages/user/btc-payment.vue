   ...
   <v-container>
     <v-col justify="center" align="center">
       ...
       <template slot-scope="props">
        <div 
            class="font-weight-bolder h6 my-3" 
            align="center" my="5" 
            justify="center" >
          <v-icon dark small>
            mdi-clock
          </v-icon> 
           Time Left • {{ props.hours }} Hrs : {{ props.minutes }} Min : {{ props.seconds }} Sec
         </div> 
       </template>
       </countdown>     
        
       <div class="my-2 body"> Scan Barcode</div> 
         <v-img class="mt-3 mb-5" 
           :src="`https://chart.googleapis.com/chart?chs=250x250&cht=qr&chl=bitcoin:${data.btc_address}?amount=${data.btc_amount}`"/>
             <v-text-field
                ...
                rounded
                readonly
                @click:prepend-inner="copy(data.btc_amount)"
                dark
                outlined
                type="text"
                />
             <v-text-field
                ...
                rounded
                readonly
                @click:prepend-inner="copy(data.btc_address)"
                dark
                outlined
                type="text"
              />
...
export default {
 ...
 methods:{
  ...
  //when user has processed payment
  success(){
